...
Method parameters may use the following keywords to force arguments to be passed as a specific type of reference, with the following variants:
...
- `in`: Argument must be initialized. The method cannot assign a new value to it. The compiler might create a temporary copy of it

`out` parameters are often used as a way of creating methods with multiple return values ([tuples](./tuples) can also be used for this purpose)
