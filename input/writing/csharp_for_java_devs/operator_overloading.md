You can overload an operator to give it a unique definition for a given class or struct. This way, Java's `a.add(b).multiply(c).subtract(d)`-style code is avoided
...
When defining an operator overload, be certain that it is actually necessary and clearly is performing the operation expected of the operator
