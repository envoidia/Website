Value types (primitives and [structs](./structs)) are stored on the stack and do not invoke the GC. Reference types (objects) are stored on the heap and do invoke the GC
...
`nint` and `nuint` use the platform they're running on (32 or 64 bit). Only use this if working directly with platform details (eg memory addresses)
